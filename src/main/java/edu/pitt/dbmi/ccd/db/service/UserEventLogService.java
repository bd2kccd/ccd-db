...
public class UserEventLogService {
    ...
    public UserEventLog logUserEvent(EventType eventType, UserAccount userAccount, Long eventLocation) {
        UserEventLog eventLog = new UserEventLog(eventType, userAccount, new Date(System.currentTimeMillis()), eventLocation);

        return userEventLogRepository.save(eventLog);
    }

    public UserEventLog logUserEvent(EventTypeName eventTypeName, UserAccount userAccount, Long eventLocation) {
        EventType eventType = eventTypeRepository.findByName(eventTypeName.name());

        return logUserEvent(eventType, userAccount, eventLocation);
    }
